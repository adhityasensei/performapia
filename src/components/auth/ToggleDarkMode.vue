<template>
  <div>
    <button @click="toggleDarkMode" class="top-0 right-0 absolute
...
<script setup lang="ts">
  ...
  const toggleDarkMode = () => {
    ...
    // You can toggle the class based on the 'isDarkMode' value
  ...
  onMounted(() => {
    if (localStorage.getItem('isDarkMode')) {
      ...
      // Update the class based on the 'isDarkMode' value when the component is mounted
      document.documentElement.classList.toggle('dark', isDarkMode.value);
    }
  });
</script>